...
def movement_create(request):
    ...
    return render(
        request, "inventory/movement_form.html", {"form": form, "heading": "Create stock movement"}
    )
...
def movement_edit(request, movement_id):
    ...
    if request.method == "POST" and form.is_valid():
        try:
            ...
            return redirect("inventory:detail", movement_id=movement.id)
    return render(
        request, "inventory/movement_form.html", {"form": form, "heading": "Edit stock movement"}
    )
...
def movement_detail_view(request, movement_id):
    context = business_context_from_request(request)
    return render(
        request, "inventory/movement_detail.html", {"movement": _movement(context, movement_id)}
    )
...
def line_create(request, movement_id):
    ...
    form = MovementLineForm(
        request.POST or None, company_id=context.company_id, movement_type=movement.movement_type
    )
    if request.method == "POST" and form.is_valid():
        try:
            add_stock_movement_line(
...
def line_edit(request, movement_id, line_id):
    ...
    if request.method == "POST" and form.is_valid():
        try:
            update_stock_movement_line(
                context, movement_id=movement.id, line_id=line.id, **_line_data(form.cleaned_data)
            )
        except ValidationError as error:
            _errors(form, error)
        else:

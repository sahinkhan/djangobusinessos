...
def purchase_orders_for_company(
...
def confirmed_purchase_orders(context: BusinessContext):
    return purchase_orders_for_company(context, status=PurchaseOrder.Status.CONFIRMED)
...
def purchase_order_detail(context: BusinessContext, *, order_id) -> PurchaseOrder:
    ...
    return _with_total(
        ...
        .prefetch_related(
            Prefetch("lines", queryset=_lines_with_receipt_quantities()),
            "receipts",
        )
...
def purchase_receipt_detail(context: BusinessContext, *, receipt_id) -> PurchaseReceipt:
    validate_business_context(context)
    return (
        PurchaseReceipt.objects.select_related("purchase_order")

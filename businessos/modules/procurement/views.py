...
def order_edit(request, order_id):
    ...
    try:
        order = PurchaseOrder.objects.select_related("supplier", "currency").get(
...
def order_detail_view(request, order_id):
    ...
    return render(
        ...
        "procurement/order_detail.html",
        {
            ...
            "can_receive": has_permission(context, RECEIVE_ORDERS),
        },
    )
...
def line_create(request, order_id):
    ...
    require_permission(context, UPDATE_ORDERS)
    try:
        order = PurchaseOrder.objects.get(id=order_id, company_id=context.company_id)
    except PurchaseOrder.DoesNotExist as exc:
        raise Http404 from exc
    if order.status != PurchaseOrder.Status.DRAFT:
...
def order_receive(request, order_id):
    context = business_context_from_request(request)
    require_permission(context, RECEIVE_ORDERS)
    try:
        order = purchase_order_detail(context, order_id=order_id)
    except PurchaseOrder.DoesNotExist as exc:
        raise Http404 from exc

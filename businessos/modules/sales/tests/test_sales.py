...
def test_concurrent_edit_does_not_recreate_a_removed_line(
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        edit_future = executor.submit(edit_line)
        remove_future = executor.submit(remove_line)
        result = edit_future.result(timeout=10)
        remove_future.result(timeout=10)

    assert result is None or isinstance(result, PermissionDenied)
    assert not SalesOrderLine.objects.filter(id=line.id).exists()
...
def test_cross_company_composition_and_reads_are_rejected(
    business_context, company, country, currency, language, uom, customer, draft_order
):
    ...
    with pytest.raises(PermissionDenied):
        list(sales_orders_for_company(other_context))
